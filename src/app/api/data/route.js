...
async function scrapePaper(link) {
  ...
  throw new Error(
    `All methods failed. Last error: ${lastError?.message || "Unknown error"}`
  );
}
...
function parseHTML(html, link) {
  try {
    ...
    $('figure img, .fig img, .image-container img, img[src*="/bin/"]').each(
      (_, el) => {
        let src = $(el).attr("src");
...
export async function GET(request) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: "Failed to read or parse data.json",
        details: err.message,
      }),
    ...
    if (index === -1) {
      return new Response(
        JSON.stringify({
          ...
          tried: titleParam,
          normalizedTried: normalizedTitleParam,
          allNormalizedTitles,
        }),
  ...
  const link = paper.Code
    ? `https://www.ncbi.nlm.nih.gov/pmc/articles/PMC${paper.Code}/`
    : null;
  if (!link) {
    return new Response(
  ...
  try {
    ...
    if (demographic === "investor") {
      ...
      const investorMatch = promptsContent.match(
        /Read the following space biology paper and summarize details relevant for a potential investor[\s\S]+?Write it as if preparing a short investor briefing rather than an academic abstract\./
      );
      promptText = investorMatch
        ? investorMatch[0]
        : "Summarize for an investor.";
    } else {
      // Extract researcher prompt
      const researcherMatch = promptsContent.match(
        /Read the following space biology paper and summarize it for a researcher and or student in the field[\s\S]+?terrestrial biomedical\s*sciences\."\*/
      );
      promptText = researcherMatch
        ? researcherMatch[0].replace(/"\*$/, "")
